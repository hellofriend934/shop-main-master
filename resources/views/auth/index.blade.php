...
@section('title', 'login')
@section('content')
<x-forms.auth-forms title="{{('login')}}" action="{{route('signIn')}}" method="POST" xmlns:x-slot="http://www.w3.org/1999/html"><form class="space-y-3">
        @csrf

        <x-forms.text-input class="space-y-3 @error('email') is-invalid @enderror" type="email" name="email" placeholder="email" required="true" value="{{old('email')}}" />


        <x-forms.text-input class="space-y-3 @error('email') is-invalid @enderror" type="password" name="password" placeholder="password" required="true" />
