    ...
    <main class="py-16 lg:py-20">
        <div class="container">
                        ...
                        <form class="space-y-8 mt-8" method="post" action="{{route('cart.add',$product)}}">
                            ...
                            <div class="flex flex-wrap items-center gap-3 xs:gap-4">
                                <div class="flex items-stretch h-[54px] lg:h-[72px] gap-2">
                                    ...
                                    <input type="quantity" name="quantity" class="h-full px-2 md:px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition" min="1" max="999" value="1" placeholder="К-во">
                                    <button type="button" class="w-12 h-full rounded-lg border border-body/10 hover:bg-card/20 active:bg-card/50 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition">+</button>
                                </div>
            ...
            <section class="mt-16 xl:mt-24">
                ...
                <!-- Products list -->
               @if(isset($watchedProducts))@foreach($watchedProducts as $wproduct)
                <div class="products grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-x-8 gap-y-8 lg:gap-y-10 2xl:gap-y-12">
                  @if($wproduct->id !== $product->id)
                        <img src=" {{$wproduct->makeThumbnail('345x320')}}">
                        {{$wproduct->title}}
                    @endif
                </div>
                @endforeach  @endif
